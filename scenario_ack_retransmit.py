...
s1 = 2/3*c # m/s
R1 = 1e6   # bps
L1 = Link("L1", distance=d1, speed=s1, lost_prob=0.15) # 15% packet loss

d2 = 1000  # m
s2 = 2/3*c # m/s
R2 = 5e5   # bps
L2 = Link("L2", distance=d2, speed=s2, lost_prob=0.15) # 15% packet loss
...
pkts = [Packet(sn=pkt_sn, size=packet_size) for pkt_sn in range(1, 11)]
hostA.send(pkts)
